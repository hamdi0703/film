...
const StatsOverview: React.FC<StatsOverviewProps> = ({ movies }) => {
  ...
  const stats = useMemo(() => {
    ...
    const avgUserRating = hasUserRatings 
        ? (userRatings.reduce((a, b) => a + b, 0) / userRatings.length).toFixed(1) 
        : null;

    // --- 2. Watch Time Calculation (Fixed for TV Series) ---
    const totalMinutes = movies.reduce((acc, m) => {
        // Check if it's a TV show (has name or episode data)
        if (m.name || m.first_air_date) {
            const episodeCount = m.number_of_episodes || 1; // Default to 1 if missing
            // Priority: episode_run_time array -> runtime -> default 40min
            const episodeRuntime = (m.episode_run_time && m.episode_run_time.length > 0) 
                ? m.episode_run_time[0] 
                : (m.runtime || 40); 
            
            return acc + (episodeCount * episodeRuntime);
        }
        
        // Movie logic
        return acc + (m.runtime || 0);
    }, 0);
            ...
            <div className="flex flex-col items-end">
                 {stats.avgUserRating ? (
                    ...
                    <>
                        ...
                        <span className="text-[10px] text-neutral-500 font-medium mr-0.5">Henüz Puan Yok</span>
                    </>
                 )}
            </div>
        </div>
        ...
        <div className="mt-2 relative z-10">
            {stats.watchTime.totalMinutes > 0 ? (
             ...
             <p className="text-[10px] text-neutral-500 mt-1 font-medium">
                 Toplam {stats.totalMovies} yapım
             </p>
        </div>
        {/* Decorative Icon */}
