...
const THEME_STYLES: Record<AnalyticsTheme, { 
    ...
    amber: {
        // FIX: Increased contrast for Amber/Yellow theme
        gradient: 'from-amber-400 to-orange-600',
        text: 'text-amber-700', // Darker for light mode visibility
...
const AnalyticsCard: React.FC<AnalyticsCardProps> = ({ title, subtitle, data, theme, type }) => {
  ...
  // Veri yoksa bile boş kart göster (Layout bozulmaması için)
  const isEmpty = !data || data.length === 0;
  const styles = THEME_STYLES[theme];
  
  const visibleData = isEmpty ? [] : (isExpanded ? data.slice(0, 10) : data.slice(0, 5));
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 rounded-3xl p-6 border border-neutral-200 dark:border-neutral-800 shadow-sm animate-fade-in flex flex-col h-full min-h-[320px]">
      ...
      <div className="space-y-5 flex-1">
        {isEmpty ? (
            <div className="flex flex-col items-center justify-center h-full text-neutral-400 opacity-50 pb-8">
            ...
            const percentage = Math.max((item.count / maxCount) * 100, 5); // Min 5% width for visibility
