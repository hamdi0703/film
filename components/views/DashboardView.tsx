...
// --- SUB-COMPONENT: EMPTY STATE (Hooks Isolation Fix) ---
// Hook'ları ana bileşenden izole etmek için alt bileşen oluşturduk.
const EmptyDashboardState: React.FC<{ 
    user: any, 
    openAuthModal: () => void, 
    createCollection: (name: string) => void 
}> = ({ user, openAuthModal, createCollection }) => {
...
const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  // --- HOOKS MUST BE AT TOP LEVEL (UNCONDITIONAL) ---
  ...
  // 1. Raw Data from Context (Safe fallback if collections is empty)
  ...
  // --- CONDITIONAL RENDER AT THE END (SAFE) ---
  if (collections.length === 0) {
      return (
          <EmptyDashboardState 
            user={user} 
